...
def recommend_books_sparse(book_name: str, top_k: int = TOP_K) -> List[str]:
    ...
    if book_name not in book_to_idx:
        print(f"❌ Book '{book_name}' not found in dataset. Try exact title from merged['book-title'].unique().")
        return []
...
try:
    ...
    print("⚠️ Model not found. Please run Cells 8–10 first.")
else:
    # Ask for a book title
    book_name = input("🔍 Enter a book title to get recommendations: ").strip()
...
print("📈 Model Evaluation Metrics (Safe Version):")
